...
# Paths (making sure it is available locally by fetching
# from S3 if missing locally)
TEST_ENGINEERED_PATH = load_from_s3(
    "processed/feature_engineered_test.csv",
    "data/processed/feature_engineered_test.csv"
)
TEST_META_PATH = load_from_s3(
...
def load_data():
    ...
    meta = pd.read_csv(TEST_META_PATH, parse_dates=["date"])[["date", "city_full"]]

    if len(fe) != len(meta):
        st.warning(" Feature Engineered and meta test lengths differ. Aligning by index.")
...
if st.button("Display Predictions"):
    ...
    if len(idx) == 0:
        st.warning("No data found for these filter selections.")
    else:
        st.write(f"Running the predictions for **{year}-{month:02d}** | Region: **{region}**")
    ...
    st.info("Choose your filters and click **Show Predictions** "
