...
# load.py - Unit Test
# Making sure that time-based splitting is working
def test_load_and_split_data_creates_splits(tmp_path):
    ...
    train, eval, test = load_and_split_data(raw_path=str(dummy_path), output_dir=tmp_path)

    assert not train.empty and not eval.empty and not test.empty
    assert train["date"].max() < pd.to_datetime("2020-01-01")
    assert eval["date"].min() >= pd.to_datetime("2020-01-01")
    assert test["date"].min() >= pd.to_datetime("2022-01-01")
    assert (tmp_path / "train.csv").exists()
    print("Test for data splitting is passed")


# preprocess.py - Unit Test
# Making sure that preprocessing functions are working correctly
def test_remove_outliers_drops_high_prices():
    df = pd.DataFrame({"median_list_price": [100_000, 500_000,
                                             20_000_000]})
    cleaned = remove_outliers(df)
    assert cleaned["median_list_price"].max() <= 19_000_000
    print("Test for outlier removal is passed")
...
def test_drop_duplicates_removes_dupes():
    ...
    print("Test for duplicate removal is passed")


def test_clean_and_merge_skips_when_city_missing():
    df = pd.DataFrame({"date": ["2020-01-01"], "price": [100]})
    result = clean_and_merge(df, metros_path=None)  # should
    # skip gracefully
    assert "date" in result.columns and "price" in result.columns
    print("Test for the clean and merge  (no city_full) passed")


# feature_engineering.py - Unit Test
# Making sure that feature functions creates consistent
# numeric features and avoids leakage
def test_add_date_features_extracts_parts():
    df = pd.DataFrame({"date": ["2020-01-15"]})
    df = add_date_features(df)
    assert (df.loc[0, "year"] == 2020 and df.loc[0, "month"] == 1
            and df.loc[0, "quarter"] == 1)
    print("Test for date extraction passed")
...
def test_frequency_encode_counts_values():
    ...
    print("Test for frequency encoding passed")


def test_target_encode_applies_mapping():
    train = pd.DataFrame({"city_full": ["A", "B", "A"],
                          "price": [100, 200, 300]})
    eval = pd.DataFrame({"city_full": ["A", "B"]})
    train, eval, te = target_encode(train, eval,
                                    "city_full", "price")
    assert "city_full_encoded" in train.columns
    assert eval["city_full_encoded"].notnull().all()
    assert te is not None
    print("Test for target encoding passed")


def test_drop_unused_columns_removes_leakage():
    tr = pd.DataFrame({
        "date": ["2020-01-01"], "city_full": ["A"],
        "zipcode": [1000], "median_sale_price": [200], "price": [300]
    })
    ev = tr.copy()
    ...
    print("Test for removing unused columns passed")


# Integration test
# Making sure the whole feature pipeline works correctly
def test_full_pipeline_integration(tmp_path):
    ...
    train, eval, test = load_and_split_data(raw_path=str(raw_path), output_dir=tmp_path)
    ...
    out_train, out_eval, out_test, freq_map, te = run_feature_engineering(
    ...
    assert te is not None
    print("Test for full pipeline integration passed")
